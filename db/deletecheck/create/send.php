...
if (isset($_SESSION["sendA"]) && $_SESSION["sendA"] == "createdelete")  {
  ...
  <body>
    ...
    if ($conn->connect_error) {
      die("<p>Es konnte keine Verbindung mit dem Datenbankserver aufgebaut werden!</p>");
          ...
          $sqlset = "INSERT INTO deletecheck (createdate, fkIdAdminUsers, fkIdUser) VALUES ('" . $createdate . "', '" . $adminid[0] . "', '" . $_SESSION["uid"] . "')";
          $sqlget = "SELECT idDelete FROM deletecheck WHERE createdate = '" . $createdate . "' AND fkIdAdminUsers = '" . $adminid[0] . "' AND fkIdUser = '" . $_SESSION["uid"] . "'";
          unset($_SESSION["uid"]);
          //setting data
          if ($conn->query($sqlset) === TRUE) {
            ...
            echo "<script> location.href='/db/deletecheck/edit/view.php?id=" . $checklistid[0] ."'; </script>";
          } else {
            echo "<p>Die Checkliste konnte nicht angelegt werden!</p>";
            echo "<a href='/db/deletecheck/create/index.php'>Anderen User auswählen</a>";
